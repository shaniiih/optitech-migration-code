...
async function migrateSapakPerComment(tenantId = "tenant_1", branchId = null) {
  ...
  try {
    ...
    while (true) {
      const [rows] = await mysql.query(
        `SELECT SapakId, prlType, Comments
           FROM tblSapakPerComments
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        for (const r of chunk) {
          ...
          values.push(
            `($${paramBase + 1}, $${paramBase + 2}, $${paramBase + 3}, $${paramBase + 4}, $${paramBase + 5}, $${paramBase + 6}, $${paramBase + 7}, $${paramBase + 8}, $${paramBase + 9}, $${paramBase + 10})`
          );
      ...
      offset += rows.length;
      console.log(`SapakPerComment migrated: ${total} (offset=${offset})`);
    }

    console.log(`✅ SapakPerComment migration completed. Total inserted: ${total}`);
  } finally {
    await mysql.end();
    await pg.end();
  }
...
module.exports = migrateSapakPerComment;
