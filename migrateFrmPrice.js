...
async function migrateFrmPrice(tenantId = "tenant_1", branchId = null) {
  ...
  try {
...

    {
      ...
      for (const row of rows) {
        const legacyId = normalizeInt(row.LabelId);
        if (legacyId !== null && !labelMap.has(legacyId)) {
          labelMap.set(legacyId, row.id);
        }
      }
    }

    {
      ...
      for (const row of rows) {
        const legacyId = normalizeInt(row.ModelId);
        if (legacyId !== null && !modelMap.has(legacyId)) {
          modelMap.set(legacyId, row.id);
        }
      }
    }
    ...
    while (true) {
      const [rows] = await mysql.query(
        `SELECT SapakId, LabelId, ModelId, Price, PubPrice, RecPrice, PrivPrice, Active, Quantity
           FROM tblFrmPrices
          ORDER BY SapakId, LabelId, ModelId
          LIMIT ? OFFSET ?`,
        [WINDOW_SIZE, offset]
      );
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        for (const row of chunk) {
          ...
          const legacyLabelId = normalizeInt(row.LabelId);
          const legacyModelId = normalizeInt(row.ModelId);
        ...
        try {
          await pg.query(
            ...
            VALUES ${values.join(",")}
            ON CONFLICT ("tenantId", "branchId", "sapakId", "labelId", "modelId")
            DO UPDATE SET
      ...
      offset += rows.length;
      console.log(`FrmPrice migrated so far: ${total} (offset=${offset})`);
    }

    console.log(`✅ FrmPrice migration completed. Total inserted/updated: ${total}`);
  } finally {
    await mysql.end();
    await pg.end();
  }
...
module.exports = migrateFrmPrice;
