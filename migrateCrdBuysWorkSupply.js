...
async function migrateCrdBuysWorkSupply(tenantId = "tenant_1", branchId = null) {
  ...
  try {
    ...
    //       SELECT 1
    //       FROM pg_indexes
    //       WHERE indexname = 'crd_buys_work_supply_tenant_supply_id_ux'
    //     ) THEN
    //       CREATE UNIQUE INDEX crd_buys_work_supply_tenant_supply_id_ux
    //       ON "CrdBuysWorkSupply" ("tenantId","workSupplyId");
    //     END IF;
    //   END$$;
    // `);
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        try {
          await pg.query(
            ...
            VALUES ${values.join(",")}
            ON CONFLICT ("tenantId", "workSupplyId")
            DO UPDATE SET
