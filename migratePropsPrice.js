...
async function migratePropsPrice(tenantId = "tenant_1", branchId = null) {
  ...
  try {
...

    {
      ...
      for (const row of rows) {
        const legacyId = normalizeInt(row.propId);
        if (legacyId !== null && !propMap.has(legacyId)) {
          propMap.set(legacyId, row.id);
        }
      }
    }
    ...
    while (true) {
      const [rows] = await mysql.query(
        `SELECT SapakID, PropId, Price, PubPrice, RecPrice, PrivPrice, Active, Quantity
           FROM tblPropsPrices
          ORDER BY SapakID, PropId
          LIMIT ? OFFSET ?`,
        [WINDOW_SIZE, offset]
      );
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        for (const row of chunk) {
          ...
          const legacyPropId = normalizeInt(row.PropId);

          const sapakId = sapakMap.get(legacySapakId) || null;
          const propId = propMap.get(legacyPropId) || null;
        ...
        try {
          await pg.query(
            ...
            VALUES ${values.join(",")}
            ON CONFLICT ("tenantId", "branchId", "sapakId", "propId")
            DO UPDATE SET
      ...
      offset += rows.length;
      console.log(`PropsPrice migrated so far: ${total} (offset=${offset})`);
    }

    console.log(`✅ PropsPrice migration completed. Total inserted/updated: ${total}`);
  } finally {
    await mysql.end();
    await pg.end();
  }
...
module.exports = migratePropsPrice;
