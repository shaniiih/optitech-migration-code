...
(async () => {
  try {
    ...
    await runStep("WorkStatus", () => migrateWorkStatus(tenantId));
    ...
    await runStep("DiagnosticProtocol", () => migrateDiagnosticProtocol(tenantId));
    await runStep("Purchase", () => migratePurchase(tenantId)); // Verified
    await runStep("Product", () => migrateProduct(tenantId)); // Verified
    await runStep("LensMaterial", () => migrateLensMaterial(tenantId));
    ...
    await runStep("FrameTrial", () => migrateFrameTrial(tenantId));
    await runStep("LowVisionManufacturer", () => migrateLowVisionManufacturer(tenantId));
    await runStep("LowVisionCap", () => migrateLowVisionCap(tenantId));
    await runStep("LowVisionArea", () => migrateLowVisionArea(tenantId));
    await runStep("GlassColor", () => migrateGlassColor(tenantId));
    await runStep("GlassRole", () => migrateGlassRole(tenantId));
    await runStep("GlassCoating", () => migrateGlassCoating(tenantId));
    await runStep("GlassMaterial", () => migrateGlassMaterial(tenantId));
