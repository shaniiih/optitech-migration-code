...
async function migrateLnsTreatmen(tenantId = "tenant_1", branchId = null) {
  ...
  try {
    ...
    while (true) {
      const [rows] = await mysql.query(
        `SELECT SapakID, TreatId, TreatCharID, Price, PubPrice, RecPrice, PrivPrice, Active
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        for (const row of chunk) {
          ...
          values.push(
            `($${params.length + 1}, $${params.length + 2}, $${params.length + 3}, $${params.length + 4}, $${params.length + 5}, $${params.length + 6}, $${params.length + 7}, $${params.length + 8}, $${params.length + 9}, $${params.length + 10}, $${params.length + 11}, $${params.length + 12}, $${params.length + 13}, $${params.length + 14}, $${params.length + 15}, $${params.length + 16})`
          );
          params.push(
            uuidv4(),        // id
            tenantId,
        ...
        try {
          await pg.query(
            ...
            VALUES ${values.join(",")}
            ON CONFLICT ("tenantId", "branchId", "sapakId", "treatId", "treatCharId")
            DO UPDATE SET
              "legacySapakId"    = EXCLUDED."legacySapakId",
              "sapakId"          = EXCLUDED."sapakId",
              "legacyTreatId"    = EXCLUDED."legacyTreatId",
              "treatId"          = EXCLUDED."treatId",
              "legacyTreatCharId"= EXCLUDED."legacyTreatCharId",
              "treatCharId"      = EXCLUDED."treatCharId",
      ...
      offset += rows.length;
      console.log(`LnsTreatmen migrated so far: ${total} (offset=${offset})`);
    }

    console.log(`✅ LnsTreatmen migration completed. Total inserted/updated: ${total}`);
  } finally {
    await mysql.end();
    await pg.end();
  }
...
module.exports = migrateLnsTreatmen;
